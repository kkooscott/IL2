...
public class IL04B01Q02 extends ActionSupport {
  private static final Logger logger = Logger.getLogger(IL04B01Q02.class);
  ...
  public String toMain() throws Exception {
    ...
    if(StringUtils.isNotEmpty(PST)){
    	PHQPST = PST;
    }else if(StringUtils.isNotEmpty(PHQ)){
    	PHQPST = PHQ;
    }
    //警局階層式選單，第一層-縣市警察局，第二層-分局
    String[] ilPsqArr = ntpcUtil.getOlderE0UnitCdByNewE0UnitCd(PHQ.toUpperCase());
    sql.append("(");
    for (int i = 0; i < ilPsqArr.length; i++) {
    	if (i == 0) {
    		sql.append("IL_PHQ='" + ilPsqArr[i] + "'");
    	} else {
    		sql.append(" OR IL_PHQ='" + ilPsqArr[i] + "'");
    	} 
    } 
    sql.append(") AND ");
    if (this.PST != null && !this.PST.equals("")) {
    	String[] ilPstArr = ntpcUtil.getOlderE0UnitCdByNewE0UnitCd(PST.toUpperCase());
        sql.append("(");
        for (int i = 0; i < ilPstArr.length; i++) {
        	if (i == 0) {
        		sql.append("IL_PST='" + ilPstArr[i] + "'");
        	} else {
        		sql.append(" OR IL_PST='" + ilPstArr[i] + "'");
        	} 
        } 
        sql.append(") AND ");
    }
    ...
    if (this.txtARC_TO != null && !this.txtARC_TO.equals(""))
      sql.append("IL_ARCTO <= '").append(this.getDateUtil.getSepSpiltDateEnd(this.txtARC_TO)).append("' and "); 
    ...
    if (this.txtAPY_TO != null && !this.txtAPY_TO.equals(""))
      sql.append("IL_APYDT <= '").append(this.getDateUtil.getSepSpiltDateEnd(this.txtAPY_TO)).append("' and "); 
    ...
    if (this.txtREAPY_TO != null && !this.txtREAPY_TO.equals(""))
      sql.append("IL_REAPYDT <= '").append(this.getDateUtil.getSepSpiltDateEnd(this.txtREAPY_TO)).append("' and "); 
    ...
    if (this.txtMISS_TO != null && !this.txtMISS_TO.equals(""))
      sql.append("IL_MISSDT <= '").append(this.getDateUtil.getSepSpiltDateEnd(this.txtMISS_TO)).append("' and "); 
    ...
    if (this.txtSBMT_TO != null && !this.txtSBMT_TO.equals(""))
      sql.append("IL_SBMTDT <= '").append(this.getDateUtil.getSepSpiltDateEnd(this.txtSBMT_TO)).append("' and "); 
    ...
    if (this.txtFIND_TO != null && !this.txtFIND_TO.equals(""))
      sql.append("IL_FNDDT <= '").append(this.getDateUtil.getSepSpiltDateEnd(this.txtFIND_TO)).append("' and "); 
    String whereCommand = "";
    String whereCommand1 = "";
    String whereCommand2 = "";logger.info(sql);
    if (sql.length() > 0)
