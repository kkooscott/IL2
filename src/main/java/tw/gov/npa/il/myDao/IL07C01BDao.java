...
public class IL07C01BDao {
  private static final Logger logger = Logger.getLogger(IL07C01BDao.class);
  ...
  public int queryoldIdBy12ArcId(long s) {
    JdbcTemplate queryd = new JdbcTemplate(this.dataSource);
    return queryd.queryForInt("SELECT IL_ARCID FROM ILTB_01_MAIN WHERE IL_12ARCID=" + 
        s);
  }
  
  public boolean queryDataBy12ArcId(long s) {
    JdbcTemplate queryd = new JdbcTemplate(this.dataSource);
    if (queryd
  ...
  public ArrayList<Iltb18DataProcessResult> queryByStatus(String status) {
    ...
    params[0] = status;
    query.query(
        "SELECT * FROM ILTB_18_DATA_PROCESS_RESULT WHERE IL_IMPST=? AND IL_IMPTP='4' ORDER BY IL_IMPDT ", 
        params, new RowCallbackHandler() {
          public void processRow(ResultSet rs) throws SQLException {
            ...
            IL07C01BDao.this.queryList.add(iltb18DataProcessResult);
          }
        });
  ...
  public ArrayList<Iltb18DataProcessResult> queryByKey(Iltb18DataProcessResultId iltb18DataProcessResultId) {
    ...
    query.query(
        "SELECT * FROM ILTB_18_DATA_PROCESS_RESULT WHERE IL_IMPDT=? AND IL_IMPTP=? AND IL_IMPTP='4'", 
        params, new RowCallbackHandler() {
          public void processRow(ResultSet rs) throws SQLException {
            Iltb18DataProcessResult iltb18DataProcessResult = new Iltb18DataProcessResult();
            Iltb18DataProcessResultId iltb18DataProcessResultId = new Iltb18DataProcessResultId();
  ...
  public void updateLog(Timestamp date, String status, int totalCount, int sucessCount, int IL_ERRTP1, int IL_ERRTP2, int IL_ERRTP3, int IL_ERRTP4, int IL_ERRTP5, int IL_ERRTP6, int IL_ERRTP7) {
    ...
    params[11] = date;
    insertd.update(
        "UPDATE ILTB_18_DATA_PROCESS_RESULT SET IL_MODIFIED=?,IL_IMPST =? ,IL_IMPTOT=?,IL_IMPOK=?,IL_ERRTP1=? ,IL_ERRTP2=? ,IL_ERRTP3=? ,IL_ERRTP4=? ,IL_ERRTP5=? ,IL_ERRTP6=? ,IL_ERRTP7=?  WHERE IL_IMPTP='4' AND IL_IMPDT=?", 
        
        params);
  }
  
  public void insertData(Iltb22IaRecord iltb22IaRecord) {
    JdbcTemplate insertd = new JdbcTemplate(this.dataSource);
    Object[] params = new Object[8];
    params[0] = iltb22IaRecord.getIlArcid();
    params[1] = iltb22IaRecord.getIlPtrldt();
    params[2] = iltb22IaRecord.getIlPtrlst();
    params[3] = iltb22IaRecord.getIlPtrlmrk();
    params[4] = iltb22IaRecord.getIlUid();
    params[5] = iltb22IaRecord.getIlUnm();
    params[6] = iltb22IaRecord.getIlLmd();
    params[7] = iltb22IaRecord.getIlMantype();
    insertd.update(
        "INSERT INTO ILTB_22_IA_RECORD (IL_ARCID,IL_PTRLDT,IL_PTRLST,IL_PTRLMRK,IL_UID,IL_UNM,IL_LMD,IL_MANTYPE) VALUES(?,?,?,?,?,?,?,?)", 
        params);
  }
  
  public void deleteData(String id) {
    JdbcTemplate insertd = new JdbcTemplate(this.dataSource);
    insertd.update("DELETE ILTB_22_IA_RECORD WHERE IL_ARCID=" + id);
  }
  ...
  public ArrayList<Iltb18DataProcessResult> queryByDate(String date, String user) {
    ...
    params[2] = user;
    query.query(
        "SELECT * FROM ILTB_18_DATA_PROCESS_RESULT WHERE IL_IMPTP='4' AND IL_IMPDT BETWEEN ? AND ? AND IL_IMPUID=?", 
        params, new RowCallbackHandler() {
          public void processRow(ResultSet rs) throws SQLException {
            Iltb18DataProcessResult iltb18DataProcessResult = new Iltb18DataProcessResult();
            Iltb18DataProcessResultId iltb18DataProcessResultId = new Iltb18DataProcessResultId();
