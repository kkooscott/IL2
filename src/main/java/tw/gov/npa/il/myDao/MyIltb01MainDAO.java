...
public class MyIltb01MainDAO extends Iltb01MainDAO {
	private static final Logger logger = Logger.getLogger(MyIltb01MainDAO.class);
	...
	public List<IL02A01U02Bean> getIL02A01U02Q(int usrFlagInfo, String ilNtcd, String ilPspt, String ilEnm,
			String txtBEGIN_DT, String ilArcno) {
		...
		try {
			...
			if (usrFlagInfo != 90 && usrFlagInfo != 91 && usrFlagInfo != 92)
				if (usrFlagInfo == 10 || usrFlagInfo == 21 || usrFlagInfo == 22 || usrFlagInfo == 32
						|| usrFlagInfo == 33 || usrFlagInfo == 50 || usrFlagInfo == 61) {
	...
	public List<IL03A01Q02Bean> getIL03A01Q02(String ilArcno, String ilPspt, String ilEnm, String ilCnm, String ilOfnm,
			String ilPhq, String ilJbpmdt, String ilJbpmdc) {
	...
	public List<Iltb01Main> getIL08A08P() {
		...
		try {
			...
			sqlStr.append(" where IL_NTCD not in ('035', '061')");
			sqlStr.append(
					" and ( ( IL_ARCTP='0' and IL_ARCST in ('2', '3') and convert(char(8), IL_ARCTO, 112) >= convert(char(8), getdate(), 112)");
	...
	public List<Iltb01Main> getIL08A10P() {
		...
		try {
			...
			sqlStr.append(
					" and (IL_ARCTP = '1' or (IL_ARCTP = '0' and convert(char(8),isnull(IL_ARCTO,''),112) >= convert(char(8),getdate(),112)))");
			sqlStr.append(" ORDER BY IL_NTCD");
			SQLQuery q = HibernateSessionFactory.getSession().createSQLQuery(sqlStr.toString());
			q.addScalar("IL_SEX", (Type) Hibernate.STRING);
			q.addScalar("IL_BTHDT", (Type) Hibernate.STRING);
	...
	public List<Iltb01Main> getIL08A22P() {
		...
		try {
			...
			sqlStr.append(
					" AND IL_PHQ   not in ('V000','W000','Y000','Z000''D000','I000','C000','B000','O000','P000')");
			sqlStr.append(" And (IL_SBMTDT is not NULL) ");
			sqlStr.append(
					" AND (IL_FNDDT is  NULL or convert(char(6),IL_FNDDT,112) = convert(char(6),dateadd(mm,-1,getdate()),112) )");
			sqlStr.append(" order by IL_NTCD");
			SQLQuery q = HibernateSessionFactory.getSession().createSQLQuery(sqlStr.toString());
			q.addEntity(Iltb01Main.class);
			list = q.list();
	...
	public List<Iltb01Main> getIL08A22PIsNew() {
		...
		try {
			...
			sqlStr.append("SELECT * FROM ILTB_01_MAIN");
			sqlStr.append(
					" WHERE IL_PHQ NOT IN ('V000','W000','Y000','Z000''D000','I000','C000','B000','O000','P000')");
	...
	public List<Iltb01Main> getIL08A27P() {
		...
		try {
			...
			sqlStr.append(
					" and ((IL_ARCTP = '0' and convert(char(8), IL_ARCTO, 112) >= convert(char(8), getdate(), 112)");
			sqlStr.append(" and IL_ARCST in ('2', '3'))");
			sqlStr.append(" or (IL_ARCTP = '1' and (IL_ARCST = '2' or (IL_ARCST = '3' and IL_MRKCS <> '2'))))");
			sqlStr.append(" order by IL_NTCD");
			SQLQuery q = HibernateSessionFactory.getSession().createSQLQuery(sqlStr.toString());
	...
	public List<Iltb01Main> getIL08A28P() {
		...
		try {
			...
			sqlStr.append(
					" and ((IL_ARCTP = '0' and convert(char(8), IL_ARCTO, 112) >= convert(char(8), getdate(), 112)) or IL_ARCTP = '1')");
			sqlStr.append(" order by IL_ARCTP");
			SQLQuery q = HibernateSessionFactory.getSession().createSQLQuery(sqlStr.toString());
			q.addEntity(Iltb01Main.class);
			list = q.list();
	...
	public List<Iltb01Main> getIL08B07P(String whereCondition) {
		...
		try {
			...
			sqlStr.append(" and IL_NTCD <> '035' and IL_NTCD <> '061'");
			sqlStr.append(
					" and IL_ARCTO >= convert(char(10), dateadd(dd, -1, convert(char(8), getdate(), 111) + '01'), 111)");
			sqlStr.append(
					" and IL_FSTAPYDT <= convert(char(10), dateadd(dd, -1, convert(char(8), getdate(), 111) + '01'), 111)");
			if (whereCondition != null && whereCondition.length() > 0) {
				sqlStr.append(" AND ");
				sqlStr.append(whereCondition);
			}
