...
public class Iltb01MainDAO extends BaseHibernateDAO {
	private static final Logger log = LoggerFactory.getLogger(Iltb01MainDAO.class);
	...
	public void save(Iltb01Main transientInstance) {
		log.debug("saving Iltb01Main instance");
		try {
	...
	public void delete(Iltb01Main persistentInstance) {
		log.debug("deleting Iltb01Main instance");
		try {
	...
	public Iltb01Main findById(Integer id) {
		log.debug("getting Iltb01Main instance with id: " + id);
		try {
			Iltb01Main instance = (Iltb01Main) getSession().get("tw.gov.npa.il.dao.Iltb01Main", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(Iltb01Main instance) {
		log.debug("finding Iltb01Main instance by example");
		try {
			List results = getSession().createCriteria("tw.gov.npa.il.dao.Iltb01Main")
	...
	public List findByProperty(String propertyName, Object value) {
		log.debug("finding Iltb01Main instance with property: " + propertyName + ", value: " + value);
		try {
			String queryString = "from Iltb01Main as model where model." + propertyName + "= ?";
	...
	public List findAll() {
		log.debug("finding all Iltb01Main instances");
		try {
			String queryString = "from Iltb01Main";
			Query queryObject = getSession().createQuery(queryString);
			return queryObject.list();
		} catch (RuntimeException re) {
	...
	public Iltb01Main merge(Iltb01Main detachedInstance) {
		log.debug("merging Iltb01Main instance");
		try {
			Iltb01Main result = (Iltb01Main) getSession().merge(detachedInstance);
	...
	public void attachDirty(Iltb01Main instance) {
		log.debug("attaching dirty Iltb01Main instance");
		try {
	...
	public void attachClean(Iltb01Main instance) {
		log.debug("attaching clean Iltb01Main instance");
		try {
	...
	public List<Iltb01Main> getIlidListByIlIlId(String ilIlId) {
		return findByProperty("ilIlId", ilIlId);
	}
}

/*
 * Location: D:\Qian\IL
 * 居留系統\20200529取得最新程式\IL2\WEB-INF\classes\!\tw\gov\npa\il\dao\Iltb01MainDAO.
