...
public class E0dtNpaunitDAO extends BaseHibernateDAO {
	...
	public static final String E0_UNIT_CD = "e0UnitCd";
	...
	public E0dtNpaunit findById(String id) {
		...
		try {
			E0dtNpaunit instance = (E0dtNpaunit) getSession().get("tw.gov.npa.il.dao.E0dtNpaunit", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(E0dtNpaunit instance) {
		...
		try {
			List results = getSession().createCriteria("tw.gov.npa.il.dao.E0dtNpaunit")
	...
	public List findByUnitFlag() {
		log.debug("finding E0dtNpaunit instance with e0UnitFlag = 10 ");
		try {
			String queryString = "from E0dtNpaunit where e0UnitFlag = '10' ";
			Query queryObject = getSession().createQuery(queryString);
			return queryObject.list();
		} catch (RuntimeException re) {
			log.error("find by unit flag failed", re);
			throw re;
		}
	}

	public List findByDeptCd(String e0DeptCd) {
		log.debug("finding E0dtNpaunit instance with e0DeptCd: " + e0DeptCd);
		try {
			String queryString = "from E0dtNpaunit where e0DeptCd = ? and e0UnitFlag = '20' ";
			Query queryObject = getSession().createQuery(queryString);
			queryObject.setParameter(0, e0DeptCd);
			return queryObject.list();
		} catch (RuntimeException re) {
			log.error("find by dept code failed", re);
			throw re;
		}
	}
	...
	public List findByE0UnitFlagSQL(String unitFlag) {
		try {
			List results = getSession().createCriteria("tw.gov.npa.il.dao.E0dtNpaunit")
					.add(Restrictions.eq("e0UnitFlag", unitFlag)).list();
			return results;
		} catch (RuntimeException re) {
			log.error("find by example failed", re);
			throw re;
