	...
	function onSubmit() {
		...
		if (!$('#resultList').find(':radio').is(':checked')) {
			...
			$('#resultList > tbody > tr').each(function(i, e) {
				...
				if ($(rdo).is(':checked')) {
					$('#txtNTCD').val(sid);
					$('#form1').attr("action", "IL07A02U03.action");
					$('#form1').submit();
				}
			});
		}
...
em {
	...
	<%@ include file="util/mask.jsp" %>
		...
		<form id="form1" name="form1" method="post" class='Form1'>
			...
			<input id="txtNTCD" name="txtNTCD" type="hidden" value="${ txtNTCD }" />
			<display:table id="resultList" name="queryList" pagesize="5" requestURI="IL07A02U02.action" decorator="tw.gov.npa.il.decorator.IL07ADecorator" cellpadding="8" style="width:100%;border-collapse:collapse;">
				<display:column headerClass="First" class="Center" property="function" title="選取" />
