...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	function onSubmit(url) {
		...
		document.form1.submit();
			$('#pageType').val('print');
			//alert($('#pageType').val()); */
			$('#form1').attr("action", "IL07D04Q02_2.action");
			$('#form1').submit();
	}
</script>
		<style type="text/css">
...
em {
	...
	String startId=(String)request.getParameter("txtSRCHSTBegin");
	String endId=(String)request.getParameter("txtSRCHSTEnd");
    %>
	...
	<%@ include file="util/mask.jsp" %>
		...
		<form id="form1" name="form1" method="post" class='Form1'>
			...
			<input id="txtInput" name="txtInput" type="hidden" value="${ txtInput }" />
			<display:table id="resultList" name="queryList" pagesize="5" cellpadding="8" 
				requestURI="IL07D04Q02.action"
				decorator="tw.gov.npa.il.decorator.IL07DDecorator"
				style="width:100%;border-collapse:collapse;">
				
				<display:column class="Center" title="筆次">${resultList_rowNum}</display:column>
				<display:column class="sid" property="SRCHST" title="特殊身份代碼" />
				<display:column class="Center" property="SRCHSTNM" title="特殊身份中文名稱" />
			</display:table>
			...
			<tr>
				<td style="text-align: center;">
					<div class="FunctionArea">
						...
						<input type="button" class='InputButton' value="列　印" onclick="onSubmit('IL07D04Q02_2.action');" />
		...
		<input id="txtSRCHSTBegin" name="txtSRCHSTBegin" type="hidden" value='<%=request.getParameter("txtSRCHSTBegin")%>'/>
		<input id="txtSRCHSTEnd" name="txtSRCHSTEnd" type="hidden" value='<%=request.getParameter("txtSRCHSTEnd")%>'/>
