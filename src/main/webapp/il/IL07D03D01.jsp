...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	function toSubmit() {
		if( !validateNull( $("#txtSRCHSTBegin").val()) ){ $.alert.open("warning", "特殊身份代碼未輸入"); }
		else if( !validateNull( $("#txtSRCHSTEnd").val()) ){ $.alert.open("warning", "特殊身份代碼未輸入"); }
		//else if( !validateFormat( $("#txtUVCDBegin").val(), 0, 4) ){ alert("大專院校代碼為四位數字，例如：0001"); }
		//else if( !validateFormat( $("#txtUVCDEnd").val(), 0, 4) ){ alert("大專院校代碼為四位數字，例如：0001"); }
		else if( $("#txtSRCHSTBegin").val() > $("#txtSRCHSTEnd").val() ){ $.alert.open("warning", "特殊身份代碼範圍輸入錯誤"); }
		else{
			document.form1.action = "IL07D03D02.action";
						...
						<fieldset>
							...
							<table width="100%" style="align:center;">
								<tr>
									...
									<td width="50%">
										<input id="txtSRCHSTBegin" name="txtSRCHSTBegin" type="text"
											class="InputText" size="2" maxLength="4" /> ~ 
										<input id="txtSRCHSTEnd" name="txtSRCHSTEnd" type="text"
											class="InputText" size="2" maxLength="4" />
									</td>
								</tr>
							</table>
				...
				<tr>
					<td>
						<div class="FunctionArea">
							...
							<input type="button" onclick="document.location='IL07D03D01.action'" name="btnClear" value="清 除" id="btnClear" class="InputButton" />
							<input type="button"
