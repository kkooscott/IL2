...
My97DP.prototype = {
    init: function () {
        ...
        if ($dp.el.nodeType == 1 && $dp.el["My97Mark"] === undefined) {
			...
			$dp.attachEvent($dp.el, "onkeyup", _tab);
    ...
    _f: function (p, c, r, e, isR) {
        ...
        for (var i = 0; i < r; i++) {
            ...
            for (var j = 0; j < c; j++) {
                ...
                var strCHYear = '';
                if (p == "y") {
                    strCHYear = ($dt[p] - 1911).toString();
                    while (strCHYear.length < 3)
                        strCHYear = '0' + strCHYear;
                }
                s.a(">" + (p == "M" ? $lang.aMonStr[$dt[p] - 1] : (p == "y" ? strCHYear : $dt[p])) + "</td>")
            }
            s.a("</tr>")
        }
        s.a("</table>");
    ...
    draw: function () {
        ...
        if ($dp.doubleCalendar) {
            ...
            var CHYear = '';
            CHYear = ($dt.y - 1911).toString();
            while (CHYear.length < 3)
                CHYear = '0' + CHYear;
            $d.yI.value = '民國  ' + CHYear;
        }
...
function _tab(M) {
	...
	if($dp.el.value.length > 0 && M.keyCode > 47){
		$dp.el.value = $dp.el.value.substring(0, $dp.el.value.length - 1);
	}
    if (Q >= 96 && Q <= 105) Q -= 48;
